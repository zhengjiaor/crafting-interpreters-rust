...
pub enum Expr {
    ...
    Set { ojbect: Box<Expr>, name: Token, value: Box<Expr> },
...
impl Parser {
    ...
    fn class_declaration(&mut self) -> Result<Stmt> {
        ...
        while !self.check(TokenType::RightBrace) && !self.is_at_end() {
            if let Stmt::Function(method) = self.function(FunctionType::Method)? {
                methods.push(method);
            }
        }
    ...
    fn assignment(&mut self) -> Result<Expr> {
        ...
        if self.match_type(&[TokenType::Equal]) {
            ...
            if let Expr::Variable(token) = expr {
                ...
                return Ok(Expr::Set { ojbect: expr, name, value: Box::new(value) });
            }
    ...
    fn call(&mut self) -> Result<Expr> {
        ...
        loop {
            if self.match_type(&[TokenType::LeftParen]) {
                ...
                expr = Expr::Get { expr: Box::new(expr.clone()), name: name.clone() };
            } else {
                break;
            }
        }
    ...
    fn primary(&mut self) -> Result<Expr> {
        ...
        if self.match_type(&[TokenType::Super]) {
            ...
            return Ok(Expr::Super { keyword, method: method.clone() });
        }
